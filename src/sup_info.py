...
generate a table with download link
...
def judgeType(link_dic):
    ...
    for name in link_dic.keys():
        if "barcode" in name.lower():
            file_type_dic["barcode"] = link_dic[name]
        elif "count" in name.lower():
            file_type_dic["count"] = link_dic[name]
        elif "tpm" in name.lower() or "matrix" in name.lower():
            file_type_dic["tpm"] = link_dic[name]
        elif "gene" in name.lower():
            file_type_dic["gene"] = link_dic[name]
        else:
...
def main():
    ...
    parser.add_argument('-x', '--xml', help='path to xml file')
    ...
    if local_flag:
        link_dic = findSuppleDataXml(xml_path)
        print(link_dic)
